...
//            Glide.with(context).load(cartProduct.getCustomersBasketProduct().getProductsImage()).into(holder.cart_item_cover);
            Glide.with(context)
                    .load(cartProduct.getCustomersBasketProduct().getProductImageX()).apply(new RequestOptions().centerCrop()
...
holder.cart_item_quantity.setTypeface(Typeface.createFromAsset(
        this.context.getAssets(),
        "font/baskvill_regular.OTF"
));

            holder.cart_item_sub_price.setTypeface(Typeface.createFromAsset(
                    this.context.getAssets(),
                    "font/baskvill_regular.OTF"
            ));

            holder.cart_item_base_price.setTypeface(Typeface.createFromAsset(
                    this.context.getAssets(),
                    "font/baskvill_regular.OTF"
            ));

            try {
                holder.cart_item_base_price.setText( (double) Double.parseDouble(cartProduct.getCustomersBasketProduct().getProductsFinalPrice()) + " " + ConstantValues.CURRENCY_SYMBOL);
                holder.cart_item_sub_price.setText( (double) Double.parseDouble(cartProduct.getCustomersBasketProduct().getTotalPrice()) + " " + ConstantValues.CURRENCY_SYMBOL);
            holder.cart_item_base_price.setTypeface(Typeface.createFromAsset(
                    this.context.getAssets(),
                    "font/baskvill_regular.OTF"
            ));
                ...
                String number_cart_item_base_price = arabicToDecimal(cartProduct.getCustomersBasketProduct().getProductsFinalPrice()).replace("٫", ".");
                String number_cart_item_sub_price = arabicToDecimal(cartProduct.getCustomersBasketProduct().getTotalPrice()).replace("٫", ".");
                holder.cart_item_base_price.setText( (double) Double.parseDouble(number_cart_item_base_price) + " " + ConstantValues.CURRENCY_SYMBOL);

                holder.cart_item_base_price.setTypeface(Typeface.createFromAsset(
                        this.context.getAssets(),
                        "font/baskvill_regular.OTF"
                ));
                holder.cart_item_sub_price.setText( (double) Double.parseDouble(number_cart_item_sub_price) + " " + ConstantValues.CURRENCY_SYMBOL);
                holder.cart_item_sub_price.setTypeface(Typeface.createFromAsset(
                        this.context.getAssets(),
                        "font/baskvill_regular.OTF"
                ));
            ...
            holder.cart_item_cover.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ...
                    fragmentManager.beginTransaction()
                            .replace(R.id.main_fragment, fragment)
                            .setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
                            .addToBackStack(context.getString(R.string.actionCart))
                            .commit();
        ...
        for (int i = 0; i < cartItemsList.size(); i++) {
            ...
            if (cartItemsList.get(i).getCustomersBasketProduct().getTotalPrice() != null) {
                try {
                    ...
                    String number = arabicToDecimal(cartItemsList.get(i).getCustomersBasketProduct().getTotalPrice());
...
//        cartFragment.cart_total_price.setText(context.getString(R.string.total) + " : " + finalPrice + " " + ConstantValues.CURRENCY_SYMBOL);
        cartFragment.cart_total_price.setText( finalPrice + " " + ConstantValues.CURRENCY_SYMBOL);
        cartFragment.total_items.setText("(" + My_Cart.getCartSize() + ")");

        cartFragment.total_items.setTypeface(Typeface.createFromAsset(
                this.context.getAssets(),
                "font/baskvill_regular.OTF"
        ));
        cartFragment.cart_total_price.setTypeface(Typeface.createFromAsset(
                this.context.getAssets(),
                "font/baskvill_regular.OTF"
        ));
